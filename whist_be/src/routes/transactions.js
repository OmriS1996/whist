...
router.get("/fivedays", async (req, res) => {
  try {
    ...
    let transactions = await query(
      SQL`select sum(transaction_usd) as total, FROM_UNIXTIME(unix_date, "%D %M, %Y") as created from transactions WHERE unix_date > ${minusFiveDays} GROUP BY year(unix_date), month(unix_date), day(unix_date);`
    );
